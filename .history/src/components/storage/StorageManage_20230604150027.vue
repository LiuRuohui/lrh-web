        ...
        methods:{
            ...
            del(id){
                this.$axios.get(this.$httpUrl+'/storage/del?id='+id).then(res=>res.data).then(res=>{
                    console.log(res)
                    if(res.code==200){
                        this.$swal("Success", "删除仓库成功","success");
                        this.loadPost()
                    }else{
                        this.$swal("Fail","删除仓库失败",'error')
                    }
            ...
            doSave(){
                this.$axios.post(this.$httpUrl+'/storage/save',this.form).then(res=>res.data).then(res=>{
                    console.log(res)
                    if(res.code==200){
                        this.$swal("Success", "添加仓库成功","success");
                        this.centerDialogVisible = false
                        this.loadPost()
                        this. resetForm()
                    }else{
                        this.$swal("Fail","添加仓库失败",'error')
                    }
            ...
            doMod(){
                this.$axios.post(this.$httpUrl+'/storage/update',this.form).then(res=>res.data).then(res=>{
                    console.log(res)
                    if(res.code==200){
                        this.$swal("Success", "修改仓库信息成功","success");
                        this.centerDialogVisible = false
                        this.loadPost()
                        this. resetForm()
                    }else{
                        this.$swal("Fail","修改仓库信息失败",'error')
                    }

        ...
        methods:{
            ...
            del(id){
                this.$axios.get(this.$httpUrl+'/goods/del?id='+id).then(res=>res.data).then(res=>{
                    console.log(res)
                    if(res.code==200){
                        this.$swal("Success", "删除商品成功","success");
                        this.loadPost()
                    }else{
                        this.$swal("Fail","删除商品失败",'error')
                    }
            ...
            doSave(){
                this.$axios.post(this.$httpUrl+'/goods/save',this.form).then(res=>res.data).then(res=>{
                    console.log(res)
                    if(res.code==200){
                        this.$swal("Success", "添加商品成功","success");
                        this.centerDialogVisible = false
                        this.loadPost()
                        this. resetForm()
                    }else{
                        this.$swal("Fail","添加商品失败",'error')
                    }
            ...
            doMod(){
                this.$axios.post(this.$httpUrl+'/goods/update',this.form).then(res=>res.data).then(res=>{
                    console.log(res)
                    if(res.code==200){
                        this.$swal("Success", "修改商品信息成功","success");
                        this.centerDialogVisible = false
                        this.loadPost()
                        this. resetForm()
                    }else{
                        this.$swal("Fail","修改商品信息失败",'error')
                    }
            ...
            doInGoods(){
                this.$axios.post(this.$httpUrl+'/record/save',this.form1).then(res=>res.data).then(res=>{
                    console.log(res)
                    if(res.code==200){
                        this.$swal("Success", "商品入库成功","success");
                        this.inDialogVisible = false
                        this.loadPost()
                        this. resetInForm()
                    }else{
                        this.$swal("Fail","商品入库失败",'error')
                    }

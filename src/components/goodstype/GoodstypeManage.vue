        ...
        methods:{
            ...
            del(id){
                this.$axios.get(this.$httpUrl+'/goodstype/del?id='+id).then(res=>res.data).then(res=>{
                    console.log(res)
                    if(res.code==200){
                        this.$swal("Success", "删除分类成功","success");
                        this.loadPost()
                    }else{
                        this.$swal("Fail","删除分类失败",'error')
                    }
            ...
            doSave(){
                this.$axios.post(this.$httpUrl+'/goodstype/save',this.form).then(res=>res.data).then(res=>{
                    console.log(res)
                    if(res.code==200){
                        this.$swal("Success", "添加分类成功","success");
                        this.centerDialogVisible = false
                        this.loadPost()
                        this. resetForm()
                    }else{
                        this.$swal("Fail","添加分类失败",'error')
                    }
            ...
            doMod(){
                this.$axios.post(this.$httpUrl+'/goodstype/update',this.form).then(res=>res.data).then(res=>{
                    console.log(res)
                    if(res.code==200){
                        this.$swal("Success", "修改分类信息成功","success");
                        this.centerDialogVisible = false
                        this.loadPost()
                        this. resetForm()
                    }else{
                        this.$swal("Fail","修改分类信息失败",'error')
                    }
